...
void main() {
  group('Chart to Trading Integration Tests', () {
    ...
    test('TradingController must implement applyAutoDetection for Chart integration', () {
      ...
      expect(() => tradingController.applyAutoDetection(decisions), returnsNormally);
      
      final tradingState = container.read(tradingProvider);
      final trendParam = tradingState.technicals.firstWhere((p) => p.title == 'Trend Alignment');
      
      expect(trendParam.isChecked, true, reason: 'Parameter should be checked after auto-detection');
      expect(trendParam.isAutoDetected, true, reason: 'isAutoDetected flag should be set to true');
    });

    test('applyAutoDetection re-calculates the score and decision correctly', () {
      final tradingController = container.read(tradingProvider.notifier);
      ...
      expect(state.totalScore, 8); // 3 + 3 + 2
